...
def _pick_latest_tce_distance_file(tables_dir: Path, scenario: str, ret: int, profile: str) -> tuple[Path, str]:
...
def main():
    ...
    rp = ensure_run_dirs_a(run_id, save_as_latest=False, meta={"script": "plot_tce_A.py"})
    ...
    src, tag = _pick_latest_tce_distance_file(rp.tables_dir, args.scenario, args.retrans, args.profile)
    ...
    plt.title(f"{args.scenario}: TCE vs Distance (ret={args.retrans}, {args.profile})")
    plt.ylim(0.0, y_top)
    ...
    if args.save_zoom:
        ...
        plt.xlabel("Distance (m)")
        plt.ylabel("Rate / Utility")
        plt.title(f"{args.scenario}: TCE vs Distance (zoom, ret={args.retrans}, {args.profile})")
