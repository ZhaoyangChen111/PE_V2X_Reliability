...
% Final figures (raw-derived) for the Day17 frozen runs.
%
% Design goal of this version:
% - Keep the original 6-figure + summary-matrix structure readable.
% - Respect the newer raw semantics where "success" is already timely success.
% - Avoid the over-aggressive rewrites that previously made the figures worse.
%
% Important note:
% - The official NEW TCE figure is generated separately by:
%       plot_tce_fig07_urbmask.m
% - This file keeps the old figure family meaningful and usable.
% - The summary matrix here is still useful as a backup / appendix page.
...
retAll = cfg.rets(:).';
retKey = [0 2];   % key rets used in several figures
...
for iS = 1:numel(cfg.scenarios)
    ...
    for k = 1:numel(retAll)
        ...
        yA = movmean(yA, 3, "omitnan");
        yB = movmean(yB, 3, "omitnan");
    ...
    text(ax, 0.02, 0.04, "Solid=NoCong, Dotted=Cong", ...
        "Units","normalized", "FontSize", 12, "Color",[0.25 0.25 0.25]);
end
...
% =============================================================================
% Fig02: Tail delays (p95 solid, p99 dotted), 2x3
% =============================================================================
% Interpretation in the current raw semantics:
% - Delay histogram is built from success==1 packets only.
% - Since success already means timely success, these are tail delays of
%   timely packets rather than all PHY-received packets.
%
% This figure still remains useful:
% - NoCong: shows the cost of retransmissions within timely-success packets.
% - Cong: shows that timely packets cluster near the business deadline band.
...
for iRun = 1:2
    ...
    for iS = 1:numel(cfg.scenarios)
        ...
        for k = 1:numel(retKey)
            ...
            % Light display smoothing to reduce jagged visual noise
            p95 = movmean(p95, 3, "omitnan");
            p99 = movmean(p99, 3, "omitnan");

            if any(isfinite(p95(focusMask))) || any(isfinite(p99(focusMask)))
                anyVisible = true;
            end
        ...
        if ~anyVisible
            text(ax, 0.5, 0.5, "No valid timely-delay bins", ...
                "Units","normalized", "HorizontalAlignment","center", ...
                "FontSize", 12, "Color",[0.35 0.35 0.35]);
        else
            text(ax, 0.02, 0.04, "p95 solid, p99 dotted", ...
                "Units","normalized", "FontSize", 12, "Color",[0.25 0.25 0.25]);
        end
    end
end
...
% =============================================================================
% Fig03: Cong-only decomposition (readable)
% =============================================================================
% Goal:
% - Keep the original engineering meaning:
%       timely = phy * (1 - late)
% - Focus on Cong only, because that is where the timeliness penalty matters most.
% - Use relaxed thresholds only here so the panels do not silently disappear.
...
for iRow = 1:2
    ...
    for iS = 1:numel(cfg.scenarios)
        ...
        % Left axis
        yyaxis(ax, "left");
        p1 = plot(ax, xDist(focusMask), phy_rate(focusMask), "-", ...
            "Color", cPhy, "LineWidth", 2.4);
        p2 = plot(ax, xDist(focusMask), timely_rate(focusMask), "-", ...
            "Color", cTimely, "LineWidth", 2.4);
        ylabel(ax, "Rate (share of total)");
        ...
        % Right axis
        yyaxis(ax, "right");
        p3 = plot(ax, xDist(focusMask), late_ratio(focusMask), ":", ...
            "Color", cLate, "LineWidth", 2.6);
        ylabel(ax, "late_ratio_phy");

        mxR = nanmax(late_ratio(focusMask));
        if ~isfinite(mxR) || mxR <= 0, mxR = 0.10; end
        ...
        if iRow == 1
            text(ax, 0.02, 0.90, "timely = phy × (1 - late)", ...
                "Units","normalized", "FontSize", 12, "Color",[0.25 0.25 0.25]);
        end
...
lg = legend(axFirst, hLegend, ...
    ["phy_rate = success_phy/total", ...
     "timely_rate = success/total", ...
     "late_ratio_phy = late/success_phy"], ...
    "Location","northoutside", "Orientation","horizontal");
lg.Box = "off";
lg.FontSize = 11;

sg = sgtitle(tl, sprintf( ...
    "Cong-only decomposition (readable): PHY success, timely success, and late penalty | ret=0 vs ret=2 (minTotal=%d, minPhy=%d)", ...
    minTotalDecomp, minPhyDecomp));
set(sg, "Interpreter","none");
...
for iM = 1:3
    ...
    fill(ax, [xf; flipud(xf)], [yLo(:); flipud(yHi(:))], [0.70 0.70 0.90], ...
        "EdgeColor","none", "FaceAlpha",0.35);
    plot(ax, xf, mu, "-", "Color",[0.15 0.15 0.60], "LineWidth", 2.8);
    ...
    text(ax, 0.02, 0.04, "Line=mean, band=±1 std across scenarios", ...
        "Units","normalized", "FontSize", 12, "Color",[0.25 0.25 0.25]);
end
...
% Upper panel: NoCong absolute heterogeneity
ax = nexttile(tl, 1);
hold(ax,"on");
for k = 1:numel(retAll)
    ret = retAll(k);
...
for k = 1:numel(retAll)
    ...
    yA = f4A.pdr_band(:);
    yA(f4A.n_total < cfg.f4.min_total) = NaN;

    yB = f4B.pdr_band(:);
    yB(f4B.n_total < cfg.f4.min_total) = NaN;
    ...
    if any(isfinite(ratio))
        ratioMax = max(ratioMax, nanmax(ratio));
    end
...
gridify(ax, cfg);
xlim(ax, [0 cfg.dist.max_m]);
ylim(ax, [0 max(0.45, min(1.0, 1.15*ratioMax))]);
xlabel(ax, "mid_x (m)");
ylabel(ax, "PDR_ratio = Cong / NoCong");
title(ax, "Congestion impact on UrbMask heterogeneity | ratio (Cong/NoCong)");
text(ax, 0.02, 0.06, "Lower ratio => stronger degradation under congestion", ...
    "Units","normalized", "FontSize", 12, "Color",[0.25 0.25 0.25]);
...
for iRun = 1:2
    ...
    for iC = 1:3
        ...
        text(ax, 0.02, 0.06, nnote, "Units","normalized", ...
            "FontSize", 12, "Color",[0.25 0.25 0.25]);
    end
end

sg = sgtitle(tl, sprintf( ...
    "Tunnel segmented effect (raw band %.0f-%.0fm): inside vs outside | bars=timely PDR, text=phy/late | unified y-lims per row", ...
...
% =============================================================================
% Fig07: Summary matrices (backup / appendix page)
% =============================================================================
% Keep this matrix because it is still very useful for:
% - quick numerical citation,
% - appendix / discussion page,
% - teacher-oriented summary page.
%
% The official NEW TCE figure is generated separately.
...
sg = sgtitle(tl, sprintf("Summary (dist<=%.0fm, weighted from raw cache) | cells: PDR, p95(ms), late(%%)", ...
    cfg.plot.focusMax_m));
set(sg, "Interpreter","none");
